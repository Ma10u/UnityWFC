...
class OverlapWFC : MonoBehaviour{
	...
	public void regenerateBlock(float x1, float y1, float x2, float y2)
	{
		...
		wavecopy = new bool[model.FMX * model.FMY][];
		for (int i = 0; i < wavecopy.Length; i++)
		{
		...
		for (int i = 0; i < FMX; i++)
		{
			for (int j = 0; j < FMY; j++)
			{
				for (int t = 0; t < model.T; t++)
				{
					...
					{
						wavecopy[i + j * FMX][t] = true;
					}
					else
					{
						wavecopy[i + j * FMX][t] = model.wave[i + j * FMX][t];
					}
				}
			}
		}
		
		model = new OverlappingModel(training.sample, N, width, depth, periodicInput, periodicOutput, symmetry, foundation, wavecopy);
		undrawn = true;
	}
